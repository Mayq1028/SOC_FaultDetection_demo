    ...
    bool parse_serial_data(const string& data, double& voltage, double& current, int& k) {
        ...
        if (data.find("<all>:") == 0) {
            try {
                ...
                if (values.size() >= 3) {
                    double U = stoi(values[2]) / 1000.0;  // 电池端电压(V)
    ...
    * --------------------------------------------------------- */
    ...
    int main() {
        ...
        if (use_serial) {
            ...
            // // 高精度计时变量
            // struct timeval tv;
            // uint64_t start_utime, end_utime;
            ...
            while (true) {
                ...
                this_thread::sleep_for(chrono::milliseconds(static_cast<int>(bp.deltaT * 415)));
                
                // // 计算精确剩余等待时间
                // gettimeofday(&tv, NULL);
                // end_utime = (tv.tv_sec * 1000000) + tv.tv_usec;
                // uint64_t elapsed = end_utime - start_utime;
                // long wait_us = static_cast<long>(bp.deltaT * 1000000) - elapsed;
                
                // // 高精度微秒级休眠
                // if (wait_us > 0) {
                //     struct timespec req = {
                //         .tv_sec = static_cast<time_t>(wait_us / 1000000),
                //         .tv_nsec = static_cast<long>((wait_us % 1000000) * 1000)
                //     };
                //     nanosleep(&req, NULL);
                // }
                // else {
                //     cerr << "警告：循环超时 " << -wait_us << " 微秒" << endl;
                // }
            }
            close(serial_fd);
        } 
...
//  当前版本可实现：
// 1. 按照等间隔进行循环，采样时间保持固定，但超过1秒
// 2. 实时读取串口/dev/ttyUSB0数据，并解析成电压与电流
// 3. 状态估计单步运行，实时预测并判断故障
// 4. 实时存储数据到文件online.csv里
